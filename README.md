...
正式版保留 Console 日誌視窗，預設不寫入 BepInEx 磁碟日誌、封包／DPS dump 或掉落診斷檔；設定保存與更新提醒狀態仍會正常保存。請一併覆蓋包內的 `BepInEx\config\BepInEx.cfg`，不要還原舊磁碟日誌設定。從 v1.6.1 升級並還原個人設定時，請將 `console_log_enabled` 設為 `true`，以恢復 Console 內的插件訊息。
