...
- **跨 Z 攻擊**：玩家角色可直接攻擊不同前後排深度的目標，減少為了對齊 Z 軸而停頓或緩慢挪動；角色不會瞬移，水平射程維持原樣。
...
新版插件 DLL 應只有 `BepInEx\plugins\AbyssSniff\AbyssSniff.dll`。v1.6.4 沿用 v1.6.3 的橋接環境，隨包提供 2026-09-09 遊戲更新後、已在本機正常進行深淵 SL 的橋接檔與配套快取，放在 `BepInEx\interop`；`Project.dll` 等橋接 DLL 不可放進插件資料夾，也不要還原舊版橋接檔。
...
跨 Z 攻擊預設開啟，受 F4 體驗優化總開關控制；也可在 `reroll_config.json` 將 `cross_z_attack_enabled` 設為 `false` 單獨停用。沿用舊設定且沒有此欄位時預設開啟，明確設為 `false` 則保留關閉。此功能不移動角色座標，保留水平射程、飛行高度相關判定、攻速與傷害規則。
...
- v1.6.4 新增玩家跨 Z 攻擊，解除追敵、起手與命中時的 Z 軸對齊限制；修正戰鬥中水晶物件清理時，復活 Buff 修補誤讀物件而導致閃退的問題。修正版已由使用者實戰確認正常，保留復活後恢復開場 Buff 的功能。
